// THIS VERSION IS THE ONE THAT DOES NOT REQUIRE FILES TO BE STORED ON GOOGLE DRIVE

// OVERVIEW: 
// This code generates an ASCII file of bibtex entries that can be used as the .bib file for a latex document. 
//
// The system is designed to be simple, to insure that what gets into the .bib file is accurate and validated information. Each pdf file
// that is brought into the "library" folder to be recorded in the bib file should be edited by the user so that the file "details" contains
// the bib entry.  Below the bib entry should be any tags that the user wishes to assign to the file, using a hashtag.  The tags should have
// corresponding folders or subfolders, where a shortcut of the pdf file will be placed. 
// The code first conducts a search for all pdf files that either have no indicators in their file description stating that 
// they have been recorded, OR that the file has an edit timestamp that is more recent than the last revision of the ascii bibtex file.
// The bibtex from those files are then processed and modified where needed in order to conform to a uniform standard, the bibtex entries
// are placed into this spreadsheet (or the entries which represent modified bibtex replace the older entries), and a new ascii file
// is generated by grabbing all the bibtex entries in this spreadsheet. 
...
function getToBeProcessedIds() {
   ...
   console.log(consts.tokens)
   console.log(consts.tokens.pmm);
...
function processTheEntries(){
   ...
   while (toBeProcessedIds && toBeProcessedIds.length > 0 && (!(outOfTime))){
      ...
      if (idType == "ads"){
          ...
          if (thisBibtex.trim() == ''){thisBibtex = adsInfo.bibtex;}
          // if the pdf link is missing, use the one provided by the ads
          if (thisPdfUrl.trim() == ''){thisPdfUrl = adsInfo.pdfUrl;}
       }
...
function displayPdf(){
   ...
   var html = HtmlService.createTemplateFromFile('pdfDisplayTemplate');
...
console.log('sourceType '+storedInfo.sourceType+'  pdfId '+storedInfo.pdfId);
...
function getInfoFromADS(adsBibcode){
   ...
   try {
       ...
       return {"bibtex":'error', "pdfUrl":'error'};
   }
   ...
   if (!pdfLinkExists){return {"bibtex":'error', "pdfUrl":'error'};}
